...
class UserController extends Controller
{
    ...
    {
        $hasPermission = false;
        $url = route('dashboard'); // Default URL
        $originalUserId = Auth::id();
        
        try {
            $user = User::where('role_id', '!=', 1)->findOrFail($user_id);
        } catch (Exception $e) {
            Toastr::error(trans('common.User not found'), trans('common.Failed'));
            return redirect()->back();
        }
        ...
        if ($user->role_id == 3) {
            ...
            // Instructor
            $url = route('dashboard');
            $hasPermission = permissionCheck('instructor.secretLogin');
            ...
            // Default to staff permission
            $url = route('dashboard');
            $hasPermission = permissionCheck('staffs.secretLogin');
        }
        
        if (!$hasPermission) {
            Toastr::error(trans('common.Permission denied'), trans('common.Failed'));
            return redirect()->back();
        }
        ...
        Toastr::success(trans('common.Login successful'), trans('common.Success'));
